...
# Browser
set -gx BROWSER /usr/bin/firefox-esr
...
function mise
  if test (count $argv) -eq 0
    command /home/linuxbrew/.linuxbrew/bin/mise
    return
  end
  ...
  if contains -- --help $argv
    command /home/linuxbrew/.linuxbrew/bin/mise "$command" $argv
    return $status
  end
  ...
  case deactivate shell sh
    ...
    if contains -- -h $argv
      ...
      source (command /home/linuxbrew/.linuxbrew/bin/mise "$command" $argv |psub)
    end
  case '*'
    command /home/linuxbrew/.linuxbrew/bin/mise "$command" $argv
  end
...
function __mise_env_eval --on-event fish_prompt --description 'Update mise environment when changing directories';
    /home/linuxbrew/.linuxbrew/bin/mise hook-env -s fish | source;
...
function fish_command_not_found
    if string match -qrv -- '^(?:mise$|mise-)' $argv[1] &&
        /home/linuxbrew/.linuxbrew/bin/mise hook-not-found -s fish -- $argv[1]
        /home/linuxbrew/.linuxbrew/bin/mise hook-env -s fish | source
